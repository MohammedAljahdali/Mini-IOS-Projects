...
class StoryNodeViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let nextStoryNodeController = self.storyboard!.instantiateViewController(withIdentifier: "StoryNodeViewController")as! StoryNodeViewController
        nextStoryNodeController.storyNode = self.storyNode.storyNodeForIndex(index: indexPath.row)
        navigationController?.pushViewController(nextStoryNodeController, animated: true)
    }
